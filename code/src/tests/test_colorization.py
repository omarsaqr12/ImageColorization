#!/usr/bin/env python3
"""
Quick Colorization Test Script
Tests both pre-trained models and creates visual comparisons
"""
...
class QuickColorizationTest:
    ...
    def load_models(self):
        ...
        print("Loading pre-trained colorization models...")
        self.colorizer_eccv16 = eccv16(pretrained=True).eval().to(self.device)
        self.colorizer_siggraph17 = siggraph17(pretrained=True).eval().to(self.device)
    ...
    def create_visual_comparison(self, samples, output_dir='colorization_test_results'):
    ...
    def create_summary_grid(self, samples, output_dir='colorization_test_results'):
        ...
        fig.suptitle('CIFAR-10 Colorization Test Results', fontsize=20, fontweight='bold')
...
def main():
    ...
    # Initialize tester
    tester = QuickColorizationTest(device=device)
    
    # Load random samples
    samples = tester.load_cifar10_samples(num_samples=10)
    
    # Create visualizations
    tester.create_visual_comparison(samples)
    tester.create_summary_grid(samples)
    
    print("\n" + "="*60)
    print("🎨 COLORIZATION TEST COMPLETE!")
    print("="*60)
    print("📁 Check the 'colorization_test_results' directory for:")
