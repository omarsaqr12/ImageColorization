#!/usr/bin/env python3
"""
ImageNet Trained Models Evaluation Script
Evaluates trained ECCV16 and SIGGRAPH17 models on ImageNet dataset
"""
...
class ImageNetTrainedEvaluator:
    ...
    def colorize_with_model(self, image_path, model, model_type='pretrained'):
        """Colorize using either pretrained or trained model"""
        try:
            ...
            print(f"Error colorizing with {model_type}: {e}")
            return None
    ...
    def evaluate_model(self, model, model_name, model_type, dataloader, num_samples=1000):
        """Evaluate a specific model"""
        print(f"\nEvaluating {model_name} ({model_type}) on ImageNet...")
        ...
        for batch_idx, (rgb_image, class_name, image_path) in enumerate(dataloader):
            ...
            try:
                ...
                colorized_img = self.colorize_with_model(image_path, model, model_type)
    ...
    def run_comprehensive_evaluation(self, imagenet_path, num_samples=1000):
        """Run comprehensive evaluation of all models"""
        print("Starting comprehensive evaluation of trained vs pretrained models on ImageNet...")
        ...
        for model, model_name, model_type in models_to_evaluate:
            summary, detailed_metrics = self.evaluate_model(
                model, model_name, model_type, dataloader, num_samples
            )
    ...
    def generate_comparison_report(self, results):
        ...
        print("\n" + "="*80)
        print("IMAGENET TRAINED vs PRETRAINED MODEL COMPARISON REPORT")
        print("="*80)
        
        # Performance Metrics Comparison
        print("\n📊 PERFORMANCE METRICS COMPARISON")
        print("-" * 80)
        print(f"{'Model':<20} {'Type':<12} {'Samples':<10} {'Success Rate':<15} {'PSNR':<8} {'SSIM':<8} {'Color Diff':<12}")
        print("-" * 80)
            ...
            for model_type in ['pretrained', 'trained']:
                ...
                if key in results:
                    ...
                    if 'success_rate' in summary and 'psnr' in summary:
                        print(f"{model_name.upper():<20} {model_type:<12} {summary['total_samples']:<10} "
                              f"{summary['success_rate']:<15.3f} "
                              f"{summary['psnr']['mean']:<8.3f} {summary['ssim']['mean']:<8.3f} "
                              f"{summary['color_diff']['mean']:<12.3f}")
                    else:
                        print(f"{model_name.upper():<20} {model_type:<12} {summary.get('total_samples', 0):<10} "
        ...
        for model_name in ['eccv16', 'siggraph17']:
            for model_type in ['pretrained', 'trained']:
                ...
                if key in results and 'inference_times' in results[key]['summary']:
                    summary = results[key]['summary']
                    print(f"{model_name.upper():<20} {model_type:<12} {summary['inference_times']['mean']:<15.2f}")
        ...
        for model_name in ['eccv16', 'siggraph17']:
            ...
            if pretrained_key in results and trained_key in results:
                ...
                if 'psnr' in pretrained and 'psnr' in trained:
    ...
    def save_results(self, results):
        ...
        results_json['metadata'] = {
            ...
            'device': self.device,
            'models_evaluated': ['eccv16_pretrained', 'siggraph17_pretrained', 'eccv16_trained', 'siggraph17_trained']
        }
        
        with open('imagenet_trained_vs_pretrained_comparison.json', 'w') as f:
            json.dump(results_json, f, indent=2)
        
        print(f"\n📄 Detailed results saved to: imagenet_trained_vs_pretrained_comparison.json")
...
def main():
    ...
    evaluator = ImageNetTrainedEvaluator(device=device)
    
    # Run comprehensive evaluation
    results = evaluator.run_comprehensive_evaluation(imagenet_path, num_samples)
    
    print("\n🎉 ImageNet trained vs pretrained evaluation completed!")
    print("Check the following files:")
    print("- imagenet_trained_vs_pretrained_comparison.json")
