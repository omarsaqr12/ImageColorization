#!/usr/bin/env python3
"""
Colorize and Compare All Models on ImageNet
Colorizes images using all available models and creates comparison visualizations.
"""
...
def create_comparison_image(original_rgb, grayscale, colorized_results, output_path, image_idx, class_name=None):
    ...
    # Create title with class name if available
    title = f'Image {image_idx + 1} - Model Comparison'
    if class_name:
        title += f'\nClass: {class_name}'
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='results/imagenet_colorization_comparisons', 
    ...
    if not os.path.exists(args.imagenet_path):
        print(f"❌ ImageNet path does not exist: {args.imagenet_path}")
        print("Please provide a valid path to the ImageNet dataset.")
        return
    ...
    print(f"\n📁 Preparing ImageNet {args.split} data ({args.num_images} images)...")
    ...
    if len(test_dataset) == 0:
        print("❌ No images found in ImageNet dataset!")
        return
    ...
    for image_idx, (rgb_image, class_name, image_path) in enumerate(tqdm(test_loader, desc="Processing images")):
        ...
        if should_save and len(colorized_results) > 0:
            ...
            class_name_str = class_name[0] if isinstance(class_name, (list, tuple)) else str(class_name)
            create_comparison_image(original_rgb, grayscale, colorized_results, output_path, image_idx, class_name_str)
            results_summary['saved_images'].append({
                'image_idx': image_idx + 1,
                'class_name': class_name_str,
                'image_path': image_path[0] if isinstance(image_path, (list, tuple)) else str(image_path)
            })
        
        # Print progress every 100 images
        if (image_idx + 1) % 100 == 0:
            print(f"\n✅ Processed {image_idx + 1}/{len(test_dataset)} images")
            if should_save:
    ...
    print("COMPLETED")
    print("="*80)
    print(f"✅ Processed {len(test_dataset)} images")
