#!/usr/bin/env python3
"""
Comprehensive Model Evaluation Script for ImageNet
Evaluates ALL models (teachers and students) with full metrics on ImageNet:
- PSNR, SSIM, LPIPS, ΔE2000, Colorfulness, KL Divergence, FID, KID
- Inference speed
...
class ImageNetDataset(Dataset):
    ...
    def __init__(self, root_dir, transform=None, max_samples=None, split='val'):
        """
        Args:
            root_dir (str): Path to ImageNet dataset directory (should point to CLS-LOC or similar)
            transform (callable, optional): Optional transform to be applied on a sample
            max_samples (int, optional): Maximum number of samples to load
            split (str): Which split to use ('train', 'val', 'test'). Default: 'val'
        """
        self.root_dir = root_dir
        self.transform = transform
        self.split = split
        
        # ImageNet CLS-LOC structure: root_dir/Data/CLS-LOC/train/ or root_dir/Data/CLS-LOC/val/
        # Check if we're already in CLS-LOC or need to navigate
        if 'CLS-LOC' in root_dir:
            # Already pointing to CLS-LOC
            base_dir = root_dir
        elif 'Data' in root_dir:
            # Pointing to Data directory, look for CLS-LOC
            base_dir = os.path.join(root_dir, 'CLS-LOC')
        else:
            # Try to find CLS-LOC in subdirectories
            base_dir = root_dir
        
        # Try different possible paths for the split
        possible_paths = [
            os.path.join(base_dir, split),  # CLS-LOC/val/
            os.path.join(base_dir, 'Data', 'CLS-LOC', split),  # Data/CLS-LOC/val/
            os.path.join(root_dir, split),  # root/val/
            base_dir  # Just use base_dir and search recursively
        ]
        ...
        for search_path in possible_paths:
            if os.path.exists(search_path):
                ...
                for ext in ['*.jpg', '*.jpeg', '*.png', '*.JPEG', '*.JPG', '*.PNG']:
                    found = glob.glob(os.path.join(search_path, '**', ext), recursive=True)
        ...
        # If still no images, try searching the root directory recursively
        if len(self.image_paths) == 0:
            print(f"No images found in standard locations, searching {root_dir} recursively...")
            for ext in ['*.jpg', '*.jpeg', '*.png', '*.JPEG', '*.JPG', '*.PNG']:
        ...
        if len(self.image_paths) == 0:
            ...
            print(f"   Please check that the ImageNet dataset is properly organized")
        else:
...
def rgb_to_lab(rgb_images, device):
...
def evaluate_model(model_info, test_loader, device, num_samples=5000, lpips_model=None):
    ...
    # Warmup
    with torch.no_grad():
        dummy_l = torch.randn(1, 1, 256, 256).to(device)  # ImageNet size
        try:
    ...
    for batch_idx, batch_data in enumerate(test_loader):
        ...
        # Handle ImageNet dataset format: (image, class_name, image_path)
        if len(batch_data) == 3:
            rgb_images, _, _ = batch_data
        else:
            rgb_images = batch_data[0]
        
        rgb_images = rgb_images.to(device)
        
        # Skip if image is invalid (all zeros from error handling)
        if torch.all(rgb_images == 0):
            continue
        ...
        try:
            ...
            print(f"   Warning: Failed to convert RGB to LAB: {e}")
            continue
        ...
        for i in range(rgb_images.size(0)):
            ...
            try:
                ...
                print(f"   Warning: Failed to process image {count}: {e}")
                continue
    ...
    if count == 0:
        raise ValueError(f"No samples were successfully processed. Check ImageNet dataset path and image loading.")
    ...
    if len(metrics['psnr']) == 0:
        raise ValueError("No metrics collected - no images were successfully processed")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Comprehensive evaluation of all models on ImageNet')
    ...
    parser.add_argument('--output_file', type=str, default='imagenet_evaluation_results.json', 
    ...
    print(f"\n📁 Preparing ImageNet dataset ({args.num_samples} samples)...")
    ...
    test_dataset = ImageNetDataset(
        root_dir=args.imagenet_path,
        transform=transform,
        max_samples=args.num_samples,
        split='val'  # Use validation set by default
    )
    
    if len(test_dataset) == 0:
        print("\n❌ ERROR: No images found in ImageNet dataset!")
        print(f"   Please verify the path: {args.imagenet_path}")
        print("   Expected structure: .../CLS-LOC/val/ or .../Data/CLS-LOC/val/")
        print("   Or images should be in subdirectories of the provided path")
        return
    ...
    print("\n" + "="*80)
    print("EVALUATION RESULTS - COMPREHENSIVE COMPARISON (ImageNet)")
    print("="*80)
    ...
    if HAS_TABULATE:
        ...
        # Simple table format
        col_widths = [40, 18, 18, 18, 18, 18, 12, 12, 20, 12, 12]
    ...
    if HAS_TABULATE:
        ...
        col_widths = [40, 20, 20, 18, 18, 12, 12]
